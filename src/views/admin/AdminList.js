...
const AdminList = () => {
  ...
  // ------------------------------------------------------------
  useEffect(() => {
    Promise.all([loadAdmins(), loadProperties()]).finally(() =>
      setLoading(false)
    );
  }, []);
  ...
  const filteredAdmins = useMemo(() => {
    return admins.filter((admin) => {
      const text = `${admin.full_name} ${admin.email} ${admin.phone} ${getPropertyName(admin.property_id)}`.toLowerCase();
    ...
    <CCard>
      <CCardHeader>
        <h4 className="d-flex justify-content-between align-items-center">
          Admin Users
          <CButton color="primary" onClick={() => navigate("/admins/create")}>
            + Add Admin
          </CButton>
        </h4>
      </CCardHeader>
